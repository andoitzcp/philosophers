...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/23 20:06:43 by acampo-p@         #+#    #+#             */
/*   Updated: 2025/08/24 14:52:40 by acampo-p@        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	jointhreads(t_prompt *prompt)
{
	...
	if (pthread_join(prompt->orchttr, NULL) != 0)
		exit_on_error(ERROR_STR_06, EXIT_MODE_PERROR);
	head = prompt->table;
	p = *head;
	while (p->rpn != *head)
	{
		if (pthread_join(p->thr, NULL) != 0)
			exit_on_error(ERROR_STR_07, EXIT_MODE_PERROR);
		p = p->rpn;
	}
	if (pthread_join(p->thr, NULL) != 0)
		exit_on_error(ERROR_STR_07, EXIT_MODE_PERROR);
...
void	create_threads(t_prompt *prompt)
{
	...
	if (pthread_create(&prompt->orchttr, NULL, control, prompt) != 0)
		exit_on_error(ERROR_STR_08, EXIT_MODE_PERROR);
	p = *(prompt->table);
	while (p->rpn != *(prompt->table))
	{
		if (pthread_create(&p->thr, NULL, routine, p) != 0)
			exit_on_error(ERROR_STR_09, EXIT_MODE_PERROR);
		p = p->rpn;
	}
	if (pthread_create(&p->thr, NULL, routine, p) != 0)
		exit_on_error(ERROR_STR_09, EXIT_MODE_PERROR);
